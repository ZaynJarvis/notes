...
import { incrementVisitCounter } from './visit-counter';
import { ZoukInteractiveBlog } from './zouk-embed';

function formatCounterValue(value, lang) {
  const numeric = Number(value);
  if (Number.isFinite(numeric)) {
    return new Intl.NumberFormat(lang === 'zh' ? 'zh-CN' : 'en-US').format(numeric);
  }
...
function useVisitCounter() {
  ...
  useEffect(() => {
    let active = true;
    setState({ status: 'loading', count: null });

    incrementVisitCounter()
      .then((payload) => {
        if (!active) return;
        if (!payload) {
          setState({ status: 'disabled', count: null });
          return;
        }
      ...
      .catch((error) => {
        if (!active) return;
        setState({ status: 'error', count: null, error: error.message });
      });
...
function Footer({ S, lang, visitCounter }) {
  return (
    <footer className="b-footer">
      <div className="b-footer__meta">
        <span>© 2026 {S.siteName}</span>
        {visitCounter?.status === 'ready' ? (
          <span className="b-footer__counter">{S.visitCount(formatCounterValue(visitCounter.count, lang))}</span>
        ) : null}
      </div>
      <div className="b-footer__icons">
        <a href="https://github.com/ZaynJarvis/notes" target="_blank" rel="noreferrer" aria-label="GitHub">
        ...
        <a href="https://x.com/zaynjarvis" target="_blank" rel="noreferrer" aria-label="X">
...
export function BlogShell({ router, lang, theme, onLang = () => {}, onToggleTheme = () => {}, S, formatDate, t, visitCounter = null }) {
  ...
  return (
    <div className="b-shell">
      ...
      <Footer S={S} lang={lang} visitCounter={visitCounter} />
...
export default function App() {
  ...
  return <BlogShell router={router} lang={lang} theme={theme} onLang={onLang} onToggleTheme={onToggleTheme} S={S} formatDate={formatDate} t={t} visitCounter={visitCounter} />;
}
